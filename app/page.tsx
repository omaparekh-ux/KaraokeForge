  ...
  return <main className="shell">
    ...
    {!job || job.status !== "complete" ? <section className="card">
      <div onClick={() => !busy && inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); if (!busy) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} className={`drop ${dragging ? "dragging" : ""}`}>
        <input ref={inputRef} className="hidden" type="file" accept="audio/*,video/*" onChange={(event: ChangeEvent<HTMLInputElement>) => choose(event.target.files?.[0])}/><div className="icon">{busy ? "◌" : "↑"}</div><h2>{file ? file.name : "Drop your song here"}</h2><p>{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB · ready to forge` : "Audio or video. We’ll handle the rest."}</p>
      ...
      {error && <div className="error">{error}</div>}{job?.status === "failed" && <button className="secondary wide" onClick={() => { setJob(null); setError(""); }}>Try again</button>}
    </section> : <section className="result"><div className="resultHeader"><div><div className="eyebrow">✓ Karaoke ready</div><h2>{job.filename || "Your karaoke video"}</h2><p>{job.wordCount ?? 0} timed words · {job.segmentCount ?? 0} lyric segments</p></div><button className="secondary" onClick={reset}>Create another</button></div><div className="player"><video controls playsInline src={`${base}/preview`}/></div><div className="actions"><a className="primary action" href={`${base}/download`}>Download MP4</a><a className="secondary action" href={`${base}/instrumental`}>Download instrumental</a><a className="secondary action" href={`${base}/lyrics`} target="_blank" rel="noreferrer">View lyrics JSON</a></div></section>}
