...
DB_PATH = Path(os.getenv("JOB_DB", str(jobs_dir / "karaokeforge.db")))
...
def create_job(job_id: str, filename: str) -> None:
    now = time.time()
    with LOCK, _db() as db:
        db.execute("INSERT INTO jobs(id,filename,status,progress,message,created_at,updated_at) VALUES (?,?,?,?,?,?,?)", (job_id, filename, "queued", 0, "Waiting to start", now, now))
        db.commit()


def update_job(job_id: str, *, status: str, progress: int, message: str, error: str | None = None) -> None:
    with LOCK, _db() as db:
        db.execute("UPDATE jobs SET status=?,progress=?,message=?,error=?,updated_at=? WHERE id=?", (status, progress, message, error, time.time(), job_id))
        db.commit()
...
def reset_running_jobs() -> None:
    with LOCK, _db() as db:
        db.execute("UPDATE jobs SET status='queued',progress=0,message='Waiting to resume',error=NULL,updated_at=? WHERE status='running'", (time.time(),))
        db.commit()
